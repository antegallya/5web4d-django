...
``` python showLineNumbers=false title="developer/views.py" ins={2,12-14} del={6-8}
...
``` python showLineNumbers=false title="developer/urls.py" ins={4} del={3}
urlpatterns = [
    path('', views.index, name='index'),
    path('<int:developer_id>/', views.detail, name='detail'),
    path('<int:pk>/', views.DevDetailVue.as_view(), name='detail'),
...
Pourquoi y a-t-il les parenthèses après `views.DevDetailVue.as_view()` et pas après `views.detail` ?
</Aside>
...
```python showLineNumbers=false title="developer/views.py" del={1-7} ins={14-17}
def index(request):
    context = {
...
``` python showLineNumbers=false title="developer/urls.py" del={2} ins={3}
urlpatterns = [
    path('', views.index, name='index'),
...
Les vues génériques sont basées sur le principe de Mixin. Wikipédia définit assez simplement ce principe de la manière suivante :
