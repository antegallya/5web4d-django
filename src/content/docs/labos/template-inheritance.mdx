...
Le gabarit a été modifié afin que les morceaux de code soient placés correctement au sein des blocs `title` et `content` défini dans le gabarit de base. Nous avons ajouté la balise `{% extends '_base.html' %}` dans le gabarit `detail.html` afin qu'il hérite du gabarit de base.


<Aside title="Exercice" icon="pencil">
Adaptez également le gabarit `index.html` afin qu'il étende le gabarit de base. Ensuite, vérifiez que votre site fonctionne toujours aussi bien qu'avant.
</Aside>

Pour plus d'information sur les balises de gabarit, lisez cette [page](https://docs.djangoproject.com/fr/4.1/ref/templates/builtins/).
